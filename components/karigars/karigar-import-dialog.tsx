...
import {
  getKarigarImportTemplate,
  importKarigarsFromExcel,
...
/**
 * Bulk-adds artisans from one spreadsheet — mirrors StockImportDialog's
 * pattern exactly. Row-level problems come back as a list and nothing is
 * created until the file is clean.
 */
export function KarigarImportDialog() {
  ...
  const handleTemplate = () => {
    startDownload(async () => {
      const template = await getKarigarImportTemplate()
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const result = await importKarigarsFromExcel(formData)
  ...
  return (
    ...
    >
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Bulk import artisans</DialogTitle>
          <DialogDescription>
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5 rounded-lg transition-colors focus-within:bg-accent/40">
            <Label htmlFor="karigar-import-file" required>Spreadsheet</Label>
            <Input
              id="karigar-import-file"
              name="file"
              type="file"
              accept=".xlsx,.xls,.csv"
              required

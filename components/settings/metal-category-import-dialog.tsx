...
/**
 * Bulk-adds Metals and Categories from one two-sheet spreadsheet — mirrors
 * StockImportDialog's pattern exactly. Row-level problems come back as a
 * list and nothing is created until the whole file is clean.
 */
export function MetalCategoryImportDialog() {
  ...
  const handleTemplate = () => {
    startDownload(async () => {
      const template = await getMetalCategoryImportTemplate()
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const result = await importMetalsAndCategoriesFromExcel(formData)
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5 rounded-lg transition-colors focus-within:bg-accent/40">
            <Label htmlFor="metal-category-import-file" required>Spreadsheet</Label>
            <Input
              id="metal-category-import-file"
              name="file"
              type="file"
              accept=".xlsx,.xls,.csv"
              required

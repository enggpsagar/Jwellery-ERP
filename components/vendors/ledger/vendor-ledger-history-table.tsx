...
function matchesSearch(entry: VendorLedgerEntryItem, query: string) {
  if (!query) return true
  const haystack = `${entry.description} ${entry.sourceType} ${entry.entryDate}`.toLowerCase()
  return haystack.includes(query)
}

function compareEntries(a: VendorLedgerEntryItem, b: VendorLedgerEntryItem, sortKey: SortKey) {
  switch (sortKey) {
    case "type":
...
export function VendorLedgerHistoryTable({ entries }: { entries: VendorLedgerEntryItem[] }) {
            ...
            {paginated.length === 0 ? (
              ...
              paginated.map((entry) => (
                <TableRow key={entry.id}>
                  ...
                  <TableCell className="text-foreground">{entry.sourceType}</TableCell>
                  <TableCell
                    className={cn(

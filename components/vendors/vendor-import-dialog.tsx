...
/**
 * Bulk-adds vendors from one spreadsheet — mirrors StockImportDialog's
...
export function VendorImportDialog() {
  ...
  const handleTemplate = () => {
    startDownload(async () => {
      const template = await getVendorImportTemplate()
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const result = await importVendorsFromExcel(formData)
    ...
    >
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            One row per vendor. <strong>Vendor Name</strong> is the only
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5 rounded-lg transition-colors focus-within:bg-accent/40">
            <Label htmlFor="vendor-import-file" required>Spreadsheet</Label>
            <Input
